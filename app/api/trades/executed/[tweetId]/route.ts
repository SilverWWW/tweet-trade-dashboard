...
export async function GET(request: NextRequest, { params }: { params: { tweetId: string } }) {
  try {
    ...
    if (!response.ok) {
      console.error(`[API] Executed trades by tweet fetch failed: ${response.status}`)
    ...
    console.error("[API] Error fetching executed trades by tweet:", error)
    return NextResponse.json({ success: false, error: "Failed to fetch executed trades" }, { status: 500 })
  }
}
