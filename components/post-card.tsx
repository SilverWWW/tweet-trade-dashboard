...
export function PostCard({
  ...
  useEffect(() => {
    console.log(`[v0] PostCard Debug - ${post.tweet_process_id}:`, {
      tweet_id: post.tweet_process_id,
      queued_trades: queuedTrades.length,
      executed_trades: executedTrades.length,
      total_trades: totalTrades,
      has_market_effect: hasMarketEffect,
      content_preview: post.tweet_content.substring(0, 50) + "...",
    })

    if (totalTrades === 0) {
      console.warn(`[v0] PostCard NO TRADES for ${post.tweet_process_id}`)
    }
  }, [
    post.tweet_process_id,
    queuedTrades.length,
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-yellow-800 mb-1">No Trade Data Found</h4>
                    <p className="text-sm text-yellow-700 mb-2">
                      This post was flagged as having market effect but no associated trades were found.
                    </p>
                    <div className="text-xs text-yellow-600 space-y-1">
                      <div>
                        • Tweet ID: <code className="bg-yellow-100 px-1 rounded">{post.tweet_process_id}</code>
                      </div>
                      <div>
                        • Market Effect: <span className="font-medium">{hasMarketEffect ? "Yes" : "No"}</span>
                      </div>
                      <div>
                        • Environment:{" "}
